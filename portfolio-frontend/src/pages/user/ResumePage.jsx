...
const TimelineItem = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute left-0 top-2 w-8 h-8 bg-gradient-to-br ${color} rounded-xl flex items-center justify-center shadow-lg transform transition-all duration-300 group-hover:scale-110`}
      >
        <Icon className="text-white text-sm" />
        <div
          className={`absolute inset-0 bg-gradient-to-br ${color} rounded-xl opacity-50 blur-lg -z-10`}
        ></div>
      </div>
        ...
        <div className="relative z-10">
          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start mb-3">
            <div className="flex-1">
              <h3 className="text-lg font-bold text-white mb-1 group-hover:text-indigo-300 transition-colors duration-300">
                {title}
              </h3>
    ...
    >
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-2">
          <span className="font-semibold text-white group-hover:text-indigo-300 transition-colors duration-300">
            {skillName}
          </span>
          <div className="flex items-center space-x-1">
            {[...Array(5)].map((_, i) => (
              ...
              >
                <FaStar
                  className={`text-xs ${
                    i < config.stars ? config.textColor : "text-gray-600"
...
const SectionHeader = ({ icon: Icon, title, subtitle, color, children }) => {
  ...
  return (
    ...
    >
      <div
        className={`inline-flex items-center justify-center w-12 h-12 bg-gradient-to-br ${color} rounded-2xl mb-4 shadow-lg`}
      >
        <Icon className="text-white text-xl" />
        <div
          className={`absolute w-12 h-12 bg-gradient-to-br ${color} rounded-2xl opacity-50 blur-lg -z-10`}
        ></div>
      </div>
...
const StatCard = ({ icon: Icon, number, label, color, index }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        <div
          className={`inline-flex items-center justify-center w-8 h-8 bg-gradient-to-br ${color} rounded-xl mb-2 shadow-lg`}
        >
          <Icon className="text-white text-sm" />
        </div>
        <motion.div
...
const ResumePage = () => {
  ...
  const { trackCertificateClick, trackSkillInteraction, trackSectionView } =
    useResumeAnalytics();
      ...
      <div className="relative z-10">
        <div className="max-w-7xl mx-auto px-4 py-12">
          ...
          >
            ...
            >
              Explore my career evolution, educational milestones, and technical
              expertise
            </motion.p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
            ...
            <div className="xl:col-span-3 space-y-12">
                  ...
                  <div className="space-y-8">
                    {experience.map((exp, idx) => (
                      <TimelineItem
                        ...
                        date={`${formatDate(exp.start_date)} - ${formatDate(
                          exp.end_date
                        )}`}
                        color="from-indigo-500 to-blue-600"
                      />
              ...
              <section
                onViewportEnter={() => trackSectionView("certifications")}
              >
                <SectionHeader
                  icon={FaTrophy}
                ...
                <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-700/50 shadow-lg">
                  <div className="text-center mb-6">
                    ...
                    <p className="text-gray-400 text-sm">
                      My expertise & proficiency levels
                    </p>
                  </div>
